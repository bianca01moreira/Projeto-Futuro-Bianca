...
function useFileUpload(options) {
  ...
  const uploadFile = async file => {
    if (file.size > options.maxSize) {
      const error = new Error(`File size exceeds maximum allowed for videos (${options.maxSize / 1024 / 1024}MB)`)
...
const DropZoneContent = ({
  ...
  <>
    <div className="tiptap-video-upload-dropzone">
      <FileIcon />
      <FileCornerIcon />
      <div className="tiptap-image-video-icon-container">
        <CloudUploadIcon />
      </div>
    </div>
